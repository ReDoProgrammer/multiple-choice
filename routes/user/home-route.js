...
router.get("/tim-kiem", (req, res) => {
  ...
  if (group) {
    //nếu biến group có giá trị => tìm kiếm group
    Group.findOne({ meta: group }, function (err, gr) {
      if (err) {
        ...
        if (!gr) {
          //nếu không tìm thấy group tương ứng
          res.render("404", {
            layout: "404",
            msg: "Không tồn tại lớp: " + group,
          });
        } else {
          if (subject) {
            //nếu có giá trị của meta subject
            Subject.findOne({ group: gr._id, meta: subject }, function (
              err,
              sbj
            ) {
              if (err) {
                //nếu có lỗi trong tìm kiếm subject
                console.log(
                  "can not find subject " +
                    subject +
                    "in group: " +
                    gr.name +
                    ". Error: " +
                ...
                if (!sbj) {
                  //nếu subject trả về không có giá trị = null?
                  res.render("404", {
                    layout: "404",
                    msg:
                      "Không tồn tại môn:" + subject + " của group: " + gr.name,
                  });
                } else {
                  //nếu tìm thấy
            ...
            //nếu không có giá trị meta subject => trả về những subject thuộc group đã tìm thấy
            Subject.find({ group: gr._id }, function (err, subjects) {
              if (err) {
                console.log(
                  "can not find any subject belongs to " +
                    gr.name +
                    ". Error: " +
                    new Error(err)
                );
